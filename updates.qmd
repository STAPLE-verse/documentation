...
Option B: Use an admin UI
	•	pgAdmin (Postgres) or phpMyAdmin (MySQL/MariaDB)
	•	Connect to your DB, open the User table, and set the role field to ADMIN.

⸻

⚠️ Security Note: Prisma Studio
	•	Do not expose Prisma Studio publicly. By default it binds to a port that is accessible to anyone.
	•	If you must use it:
	•	Bind to localhost and tunnel over SSH, or
	•	Put it behind a protected VPN/reverse proxy with authentication.
...
Migration failed
	•	Review the error and the specific migration folder under prisma/migrations.
	•	Restore from backup if needed, fix data issues, and re-run.

Static files/styles look stale
	•	Ensure blitz build completed without errors.
	•	Clear CDN or proxy caches if applicable.

⸻

Rollback (Quick Outline)
